...
public interface IInventorySnapshotKeyCodec<TKey>
{
    ...
    bool TryEncode(TKey value, out SnapshotValue? encoded, out string? error);

    /// <summary>Attempts to decode a supported historical version.</summary>
    bool TryDecode(SnapshotValue encoded, int version, out TKey value, out string? error);
}
...
internal interface ISnapshotValueCodec<T>
{
    string FormatId { get; }
    int CurrentVersion { get; }
    bool TryEncode(T value, out SnapshotValue? encoded, out string? error);
    bool TryDecode(SnapshotValue encoded, int version, out T value, out string? error);
}
