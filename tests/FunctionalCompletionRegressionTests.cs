...
public class FunctionalCompletionRegressionTests
{
    ...
    public void MultiCellGridLayout_FailedCompactSortIsAtomic()
    {
        ...
        Assert.That(inventory.TrySortLayout(MultiCellGridSortContext<string>.Compact(), out var failure), Is.False);
        Assert.That(failure?.Message, Is.EqualTo("Not enough empty grid space for sorted layout."));
    ...
    public void MultiCellGridLayout_BottomLeftAnchorPlacesFootprintAboveCoordinate()
    {
        ...
        Assert.That(inventory.TryAdd(table, out var failure, 1, MultiCellGridLayoutContext<string>.Single(1, 2, GridAnchor.BottomLeft)), Is.True);
    ...
    public void EquipmentLayout_TrySortReturnsUnsupported()
    {
        ...
        Assert.That(inventory.TrySortLayout((a, b) => string.CompareOrdinal(a.Definition.Id, b.Definition.Id), out var failure), Is.False);
        Assert.That(failure?.Message, Is.EqualTo("Layout does not support sorting."));
    }
