...
public class TKeyOverloadTests
{
    ...
    public void InventoryTKeyOverloads_ResolveCurrentAndMigratedStringIds()
    {
        ...
        Assert.That(inventory.TryAdd("old-apple", out var addError, amount: 5), Is.True, addError);
        ...
        Assert.That(inventory.TryRemoveByDefinition("old-apple", amount: 2, ignoreMetadata: true, out var removeError), Is.True, removeError);
    ...
    public void InventoryTKeyOverloads_UnknownIdsUseTryAndThrowingSemantics()
    {
        ...
        Assert.That(addError, Does.Contain("could not be resolved"));

        Assert.That(inventory.TryRemoveByDefinition("missing", amount: 1, ignoreMetadata: true, out var removeError), Is.False);
        Assert.That(removeError, Does.Contain("could not be resolved"));

        Assert.Throws<InvalidOperationException>(() => inventory.Add("missing"));
        Assert.Throws<InvalidOperationException>(() => inventory.Count("missing"));
        Assert.Throws<InvalidOperationException>(() => inventory.Contains("missing"));
        Assert.Throws<InvalidOperationException>(() => inventory.Find("missing"));
        Assert.Throws<InvalidOperationException>(() => inventory.RemoveByDefinition("missing", amount: 1, ignoreMetadata: true));
    }
    ...
    public void InventoryTKeyOverloads_WorkWithIntegerKeysAndContexts()
    {
        ...
        Assert.That(inventory.TryAdd(100, out var error, amount: 3, context: SlotLayoutContext<int>.Single(1)), Is.True, error);
    ...
    public void TransactionBuilderTKeyOverloads_AddAndRemoveResolvedDefinitions()
    {
        ...
        Assert.That(builder.TryAdd("sword", out var addError, amount: 1, context: EntryLayoutContext<string>.Single(1)), Is.True, addError);
        Assert.That(builder.TryRemoveByDefinition("old-apple", amount: 1, ignoreMetadata: true, out var removeError), Is.True, removeError);
        Assert.That(inventory.TryCommitTransaction(builder, out var commitError), Is.True, commitError);
    ...
    public void TransactionBuilderTKeyOverloads_SupportMetadataAddOverload()
    {
        ...
        Assert.That(builder.TryAdd("apple", amount: 1, context: null, metadata, out var error), Is.True, error);
        Assert.That(inventory.TryCommitTransaction(builder, out var commitError), Is.True, commitError);
    ...
    public void TransferBuilderTKeyOverload_RemovesByResolvedDefinitionId()
    {
        ...
        Assert.That(builder.TryRemoveByDefinition("old-apple", amount: 3, ignoreMetadata: true, out var error), Is.True, error);
        Assert.That(source.TryCommitTransfer(builder, target, out var commitError), Is.True, commitError);
    ...
    public void BuilderTKeyOverloads_UnknownIdsReturnFalse()
    {
        ...
        Assert.That(transferError, Does.Contain("could not be resolved"));
    }
