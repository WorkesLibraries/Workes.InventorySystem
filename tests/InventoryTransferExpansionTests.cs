...
public class InventoryTransferExpansionTests
{
    ...
    [Test]
    public void TryTake_RemovesFromSimulationOnly()
    {
        ...
        var result = builder.TryTake(source.Items[0], 2, out var error);
    ...
    [Test]
    public void TryTake_RejectsInvalidAmountAndDetachedItem()
    {
        var manager = CreateManager();
        var apple = new ItemDefinition<string>("apple");
        manager.Registry.Register(apple);
        ...
        Assert.That(builder.TryTake(detached, 1, out var detachedError), Is.False);
        Assert.That(detachedError, Is.EqualTo("Item not found in inventory."));
        Assert.That(builder.TryTake(detached, 0, out var amountError), Is.False);
        Assert.That(amountError, Is.EqualTo("Amount must be greater than zero."));
    }

    [Test]
    public void TryTakeAtStorageIndex_RejectsInvalidIndex()
    {
        var manager = CreateManager();
        manager.Registry.Freeze();
        var builder = InventoryTransfer.From(manager.CreateInventory());

        Assert.That(builder.TryTakeAtStorageIndex(0, 1, out var error), Is.False);
        Assert.That(error, Is.EqualTo("Index out of range."));
    }

    [Test]
    public void TryTakeByDefinition_RespectsIgnoreMetadata()
    {
        ...
        var seed = source.CreateTransactionBuilder();
        seed.TryAdd(apple, out _, 3);
        seed.TryAdd(apple, 2, null, metadata, out _);
        source.CommitTransaction(seed.ToInventoryTransaction());

        var builder = InventoryTransfer.From(source);
        Assert.That(builder.TryTake(apple, 5, ignoreMetadata: true, out var error), Is.True, error);
    ...
    public void TryTransfer_WithBuilder_CommitsOneSourceAndOneTargetEvent()
    {
        ...
        var builder = InventoryTransfer.From(source);
        builder.TryTake(source.Find(apple).Single(), 2, out _);
        builder.TryTake(source.Find(berry).Single(), 1, out _);
        int sourceEvents = 0;
        int targetEvents = 0;
    ...
    public void TryTransfer_WithBuilder_TargetRuleFailureLeavesSourceUnchanged()
    {
        ...
        var builder = InventoryTransfer.From(source);
        builder.TryTake(source.Items[0], 1, out _);
    ...
    public void TrySwap_SucceedsForPartialAmountsAndPreservesMetadata()
    {
        ...
        var seed = second.CreateTransactionBuilder();
