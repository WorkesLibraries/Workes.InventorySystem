...
public class UiMetadataChangedEventExampleTests
{
    ...
    public void UsesMetadataChangedPayloadsForTargetedUiUpdates()
    {
        ...
        Assert.That(gemStack.Metadata.TrySet("quality", "polished", out var failure), Is.True);
        Assert.That(gemStack.Metadata.TrySet("owner", "player", out failure), Is.True);
        Assert.That(gemStack.TrySplitAndSetMetadata(1, "quest-item", true, out var questGem, out failure), Is.True);
